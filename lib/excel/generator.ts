...
export async function generateExcelOutput(
  ...
  row++; // spacer
  ws.mergeCells(row, 1, row, COLS);
  const disclaimerCell = ws.getCell(row, 1);
  disclaimerCell.value =
    "⚠  AI-Generated Content: This document has been generated by artificial intelligence based on the provided "
    + "meeting transcript. Users must review and verify all information — including names, dates, decisions, and action "
    + "items — for accuracy and completeness before sharing on any platform or using for official purposes.";
  disclaimerCell.font = { bold: false, italic: true, size: 9, color: { argb: "FF92400E" } };
  disclaimerCell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFFFFBEB" } };
  disclaimerCell.alignment = { vertical: "middle", wrapText: true, horizontal: "left" };
